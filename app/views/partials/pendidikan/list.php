...
?>
<template id="pendidikanList">
    <section class="page-component">
        <div v-if="showheader" class="bg-light p-3 mb-3">
            <div class="container-fluid">
                <div class="row ">
                    <div  class="col-sm-4 comp-grid" :class="setGridSize">
                        <h3 class="record-title">Pendidikan</h3>
                    </div>
                    <div  class="col-sm-3 comp-grid" :class="setGridSize">
                        <?php 
                        if($can_add){
                        ?>
                        <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/pendidikan/add'">
                        ...
                        <div  class=" animated fadeIn">
                            <div class="">
                                <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                    <ul class="breadcrumb m-0 p-2">
                                        <li v-if="fieldname" class="breadcrumb-item">
                                            <router-link class="text-capitalize" to="/pendidikan">Pendidikan</router-link>
                                        </li>
                                            ...
                                            <tr v-for="(data,index) in records">
                                                ...
                                                <th class="td-btn">
                                                    <div >
                                                        ...
                                                        if($can_view){
                                                        ?>
                                                        <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/pendidikan/view/' + data.id">
                                                        <i class="fa fa-eye"></i> 
                                                        </router-link>
                                                        <?php 
                                                        }
                                                        ...
                                                        if($can_edit){
                                                        ?>
                                                        <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/pendidikan/edit/' + data.id">
                                ...
                                <div v-if="showfooter" class="page-footer">
                                    ...
                                    <dataimport extensions="" buttontext="" post-action="pendidikan/import_data" v-if="importbutton"></dataimport>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
	var PendidikanListComponent = Vue.component('pendidikanList', {
		template: '#pendidikanList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'pendidikanlist',
			},
			apipath : {
				type : String,
				default : 'pendidikan/list',
			},
			tablestyle: {
				type: String,
