...
?>
<template id="kinerja_prestasiList">
    <section class="page-component">
        <div v-if="showheader" class="bg-light p-3 mb-3">
            <div class="container-fluid">
                <div class="row ">
                    <div  class="col-sm-4 comp-grid" :class="setGridSize">
                        <h3 class="record-title">Kinerja Prestasi</h3>
                    </div>
                    <div  class="col-sm-3 comp-grid" :class="setGridSize">
                        <?php 
                        if($can_add){
                        ?>
                        <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/kinerja_prestasi/add'">
                        <i class="fa fa-plus"></i>
                        Add New Kinerja Prestasi
                        </router-link>
                        <?php 
                        }
                        ?>
                        ...
                        <div  class=" animated fadeIn">
                            <div class="">
                                <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                    <ul class="breadcrumb m-0 p-2">
                                        <li v-if="fieldname" class="breadcrumb-item">
                                            <router-link class="text-capitalize" to="/kinerja_prestasi">Kinerja Prestasi</router-link>
                                        </li>
                                    ...
                                    <table class="table" :class="tablestyle">
                                        ...
                                        <tbody>
                                            <tr v-for="(data,index) in records">
                                                ...
                                                <td><router-link :to="'/kinerja_prestasi/view/' +  data.id">{{data.id}}</router-link></td>
                                                <th class="td-btn">
                                                    <div >
                                                        <?php 
                                                        if($can_view){
                                                        ?>
                                                        <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/kinerja_prestasi/view/' + data.id">
                                                        <i class="fa fa-eye"></i> 
                                                        </router-link>
                                                        <?php 
                                                        }
                                                        ...
                                                        if($can_edit){
                                                        ?>
                                                        <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/kinerja_prestasi/edit/' + data.id">
                                ...
                                <div v-if="showfooter" class="page-footer">
                                    ...
                                    <dataimport extensions="" buttontext="" post-action="kinerja_prestasi/import_data" v-if="importbutton"></dataimport>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
	var Kinerja_PrestasiListComponent = Vue.component('kinerja_prestasiList', {
		template: '#kinerja_prestasiList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'kinerja_prestasilist',
			},
			apipath : {
				type : String,
				default : 'kinerja_prestasi/list',
			},
			tablestyle: {
				type: String,
